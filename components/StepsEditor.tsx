...
export default function StepsEditor({ type, recipe, onSave, onCancel }: Props) {
  const displayText = type === "recipeIngredient" ? "ingredients" : "steps";

  function parseRecipeData() {
    if (type === "recipeInstructions") {
      const instructions = recipe[type] as HowToStep[] | HowToSection[];

      const arr: string[] = [];

      instructions.forEach((step) => {
        if (step["@type"] === "HowToSection") {
          const section = step.itemListElement as HowToStep[];
          section.forEach((item) => arr.push(String(item.text)));
        } else {
          arr.push(String(step.text));
        }
      });
